...
- **Secciones extra del formulario genérico** (`core/entities/formExtension.ts` + `features/entity-crud/form/formExtensions.ts`): una entidad conserva el formulario dinámico de sus campos y suma secciones propias, cada una en su pestaña junto a "Datos" (todas montadas), que validan antes de guardar y guardan después (`afterSave` con el registro ya creado). Al guardar se muestra la pestaña que bloquea o falla. Si el croquis falla después de crear el bus, el formulario pasa a la edición del bus y el borrador del croquis se recupera.
