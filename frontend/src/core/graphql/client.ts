/**
 * Cliente GraphQL de API Platform (singleton `graphql`).
 *
...
export class ApiPlatformClient {
  ...
  constructor(options: ApiPlatformClientOptions = {}) {
    this.uri = options.uri ?? config.graphqlUrl;
  ...
  async item<T>(entity: EntitySchema, id: string | number, fields?: string[]): Promise<T> {
    if (!entity.queryItem) throw new Error(`[graphql] "${entity.name}" no expone query item`);
  ...
  async collection<T>(
    ...
    if (!entity.queryCollection)
      throw new Error(`[graphql] "${entity.name}" no expone query collection`);
  ...
  private async runMutation<T>(
    ...
    if (!mutation) throw new Error(`[graphql] "${entity.name}" no expone mutación`);
...
/** Cliente GraphQL de la aplicación. */
export const graphql = new ApiPlatformClient();
