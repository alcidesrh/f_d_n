...
import type { AgnosticOption, CollectionResult, EntitySchema, MutationSchema } from "@/core/graphql/types";
...
export const repository = {
  ...
  async collection<T>(store: EntityStore<T>): Promise<CollectionResult<T>> {
    ...
    const order = entity.orderInput
      ? store.order.filter((cond) => Object.keys(cond).every((field) => entity.orderFields.includes(field)))
      : [];
    ...
    if (page) store.pagination = { ...result.pagination, itemsPerPage: page.itemsPerPage || result.pagination.itemsPerPage };
    return result;
  },
  ...
  async update<T>(store: EntityStore<T>, data: Record<string, unknown>): Promise<T> {
    ...
    if (id !== undefined) store.items = store.items.map((item) => (idOf(item) === id ? updated : item));
    return updated;
  },
