    ...
    setQueries(schema, entity: Entity) {
      ...
      if (temp) {
        ...
        temp.args.forEach((v) => {
          args[v.name] = {
            type:
              (v.type.name || v.type.ofType?.name) +
              (v.type.kind == "NON_NULL" ? "!" : ""),
          };
        });
        itemQuery = {
          name: temp.name,
      ...
      if (temp) {
        ...
        let t = schema.types.find(
          (v) => v.name == (temp.type.name || temp.type.ofType?.name),
        );
        let f = [];
        t.fields.map((v) => {
          f.push({
      ...
      const mutationsTypes = schema.types
        ...
        .forEach((element: QueryType) => {
          ...
          element.args.forEach((v) => {
            arg[v.name] = {
              type:
                v.type.ofType?.name + (v.type.kind == "NON_NULL" ? "!" : ""),
            };
          });
          // cl(arg);
          // }
    ...
    getInput(f: Field) {
      ...
      if (f.type.kind == "SCALAR") {
        ...
        if (f.type.kind == "LIST") {
          ...
          f.type.kind == "OBJECT" &&
          f.type.name.endsWith("PageConnection")
        ) {
