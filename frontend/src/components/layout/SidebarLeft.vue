  ...
  <aside :class="sidebarClasses" :style="sidebarStyle">
    <div class="sidebar-scroll">
      ...
      <nav class="nav">
        ...
        <router-link v-for="item in NAV_MAIN" :key="item.to" :to="item.to" class="nav-item" :data-tip="item.label">
      ...
      <nav class="nav">
        <div class="nav-rail" aria-hidden="true"></div>
        <router-link v-for="item in NAV_OPS" :key="item.to" :to="item.to" class="nav-item" :data-tip="item.label">
          <span class="stop-dot" aria-hidden="true"></span>
          <span class="nav-ic"><AppIcon :name="item.icon" :size="18" /></span>
          <span class="nav-label">{{ item.label }}</span>
          <span v-if="item.count" class="nav-count">{{ item.count }}</span>
    ...
    <div class="sidebar-footer">
      <div class="state-switch" role="radiogroup" aria-label="Estado del panel izquierdo">
        <button :class="{ active: ui.leftState === 'close' }" title="Cerrar" @click="ui.setLeft('close')">
          <AppIcon name="x" :size="15" />
        </button>
        <button :class="{ active: ui.leftState === 'mini' }" title="Minimizado" @click="ui.setLeft('mini')">
          <AppIcon name="minus" :size="15" />
        </button>
        <button :class="{ active: ui.leftState === 'open' }" title="Expandido" @click="ui.setLeft('open')">
