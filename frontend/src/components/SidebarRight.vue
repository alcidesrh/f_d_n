<template>
  <aside class="right-sidebar">
    <div
      @click="sidebarStore.setMode()"
      class="sidebar-right-trigger"
      :class="{ close: sidebarStore.mode === 'close' }"
    >
    ...
    <div class="sidebar-panel">
      <!-- Large mode -->
      <div
        v-if="sidebarStore.mode === sidebarStore.modeStates.large"
        class="panel-content"
      >
        ...
        <SectionTree
          v-for="(sec, i) in filteredSections"
          :key="i"
          :item="sec"
          :depth="0"
          @action="handleAction"
      ...
      <!-- Mini mode -->
      <div
        v-if="sidebarStore.mode === sidebarStore.modeStates.mini"
        class="panel-mini"
      >
        <div class="mini-cp-wrapper">
          <icon
            name="manage_accounts"
            class="mini-icon cp-highlight"
    ...
    >
      <div class="dev-row">
        <q-btn
          ...
          dense
          icon="sym_o_sync"
          label="Refrescar Entidades"
          class="full-width dev-btn"
          @click="refreshSchema"
        />
      </div>
      <div class="dev-row entity-row">
        ...
        <q-btn
          ...
          icon="sym_o_refresh"
          class="dev-icon-btn"
          @click="refreshEntityStore"
          :disable="!selectedEntity"
        />
      </div>
    </div>
    <div
      v-else-if="sidebarStore.mode === sidebarStore.modeStates.mini"
      class="sidebar-dev-controls sidebar-dev-controls-mini"
    >
...
async function refreshSchema() {
  try {
    ...
    $q.notify({
      type: "positive",
      message: "Metadata de entidades actualizada",
      timeout: 2000,
    });
  } catch {
    $q.notify({
      type: "negative",
      message: "Error al refrescar metadata",
      timeout: 3000,
    });
  }
}
...
async function refreshEntityStore() {
  ...
  try {
    ...
    if (store) {
      $q.notify({
        type: "positive",
        message: `Store "${selectedEntity.value}" actualizada`,
        timeout: 2000,
      });
    }
  } catch {
    $q.notify({
      type: "negative",
      message: `Error al refrescar "${selectedEntity.value}"`,
      timeout: 3000,
    });
  }
}
