<template>
	<div :id="`drag-${type}`" class="drag-container bg-surface-2" :style="{ height: `${h}px` }">
		<div v-for="(element, index) in localFields" :key="index" class="field-item w-full" :ref="`itemRefs${type}`">
			<div class="row items-center no-wrap h-full flex justify-between bg-white field-item-content">
...
let cells = [],
	...
	// colSize = 1025,
	container
const h = ref(localFields.value.length * 60 + 50)
