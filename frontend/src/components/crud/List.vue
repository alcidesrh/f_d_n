...
<template>
  <div>
    <div v-if="store.metadata" class="card flex flex-col overflow-hidden" style="min-height: 400px">
      ...
      <Toolbar class="rounded-none border-none! bg-transparent px-2">
        ...
        <template #end>
          <div class="flex items-center gap-1">
            <Button
              :icon="selectionMode ? 'pi pi-check-square' : 'pi pi-square'"
              rounded
              text
              severity="secondary"
              size="small"
              :aria-label="selectionMode ? 'Desactivar selección' : 'Modo selección'"
              @click="toggleSelection"
            />
            <!-- <div v-if="store.hiddenColumns > 0" class="relwative"> -->
            <OverlayBadge
            ...
            >
              <!-- <i
                class="pi pi-eye-slash p-[5px] cursor-pointer"
      ...
      >
        <Column
          v-for="(col, i) in store.visibleColumns"
          :key="col.field"
          :field="col.field"
