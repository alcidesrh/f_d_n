<template>
  <div v-if="store" class="flex justify-between w-full ml-auto mb-20px">
    <span data-tootik="Tootik Text" data-tootik-conf="shadow right">{{
      store.name
    }}</span>
    <div class="flex table-options text-surface-2">
      <div
        @click="$router.push({ path: `/form/${store.nameDecapitalize}` })"
        :data-tootik="`Agregar ${store.name}`"
        data-tootik-conf=""
      >
        <icon name="add" />
      </div>
          ...
          >
            ...
            >
              <q-card-section>
                ...
                <template
                  v-for="(col, i) in store.config.collectionFieldConfig.filter(
                    (v) => v.visible,
                  )"
                  :key="i"
                >
...
onBeforeMount(async () => {
  ...
  visibleAllColumns.value =
    store.value.visibleColumns.length == store.value.columns.length;
