...
function injectSfjs() {
  ...
  const hasClass = (el: Element, cssClass: string) =>
    el.classList.contains(cssClass);
  const removeClass = (el: Element, cssClass: string) =>
    el.classList.remove(cssClass);
  const addClass = (el: Element, cssClass: string) =>
    el.classList.add(cssClass);
  const toggleClass = (el: Element, cssClass: string) =>
  ...
  const setPref = (name: string, value: string) =>
    localStorage.setItem(psk + name, value);
  const addEventListener = (el: any, event: string, cb: any, opts?: any) =>
    el.addEventListener(event, cb, opts || false);
  ...
  const initToolbar = (token: string) => {
    ...
    const blocks = document.querySelectorAll(
      `#sfwdt${token} .sf-toolbar-block`,
    );
    for (let i = 0; i < blocks.length; i++) {
      (function (block) {
        addEventListener(block, "mouseenter", () =>
          handleBlockHover(block, "show"),
        );
        addEventListener(block, "mouseleave", () =>
          handleBlockHover(block, "hide"),
        );
      })(blocks[i]);
    }
  };
