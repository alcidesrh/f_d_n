...
export type CollectionKind =
  "page-connection" | "cursor-connection" | "list" | "single";
...
/**
 * Opción de `fullList` (query `collectionAgnostic`).
 * El backend devuelve `{ value: IRI, label }`; algunos consumidores escriben
 * `{ id: IRI, label }`. El serializer normaliza con `value ?? id`.
 */
