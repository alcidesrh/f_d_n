<template>
  <div ref="logosEl" class="background">
    <div v-for="src in LOGOS" :key="src" class="img bg-surface-300/50" :class="bgClass" :style="{ backgroundImage: `url('${src}')` }" />
  </div>
  <div class="overlay" />
...
function advance() {
  ...
  timeline = gsap
    ...
    .to(from, { y: -280, rotation: 5, scale: 0.95, opacity: 0, duration: 0.4, ease: "back.in(1.7)" }, 0)
...
.background .img {
  ...
  &:nth-child(1) { transition: border 0.2s 0.5s, background-color 0.2s 0.5s; }
  &:nth-child(2) { transition: border 0.2s 0.7s, background-color 0.2s 0.7s; }
  &:nth-child(3) { transition: border 0.2s 0.9s, background-color 0.2s 0.9s; }
  &:nth-child(4) { transition: border 0.2s 1.12s, background-color 0.2s 1.12s; }
  &:nth-child(5) { transition: border 0.2s 1.24s, background-color 0.2s 1.24s; }
}
</style>
