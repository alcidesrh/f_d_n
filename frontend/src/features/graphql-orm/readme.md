...
```
frontend/
  src/features/graphql-orm/
    core/                      # @graphql-orm/core   — núcleo SIN dependencia de Vue
      src/
    ...
    vue/                       # @graphql-orm/vue    — integración Vue 3 + TanStack Query
      src/
        plugin.ts
        ...
        index.ts
  src/
    types/entities/            # EntityMap: tipos estáticos por entidad (capa DX)
    main.ts                    # app.use(VueQueryPlugin); app.use(createGraphQLOrm(...))
    features/graphql-orm/views/GraphQLOrmDemo.vue   # demo funcional (ruta /demo-graphql)
  smoke-test.ts                # smoke test E2E contra backend real (npx jiti smoke-test.ts)
```

> **Importante:** estos paquetes **no son paquetes npm reales** (no hay `package.json` dentro de la feature). Se exponen como módulos fuente vía alias:
> - `vite.config.ts`: `@graphql-orm/core` → `./src/features/graphql-orm/core/src/index.ts`, `@graphql-orm/vue` → `./src/features/graphql-orm/vue/src/index.ts`
> - `tsconfig.app.json`: `paths` equivalentes.
>
> La capa `vue` importa el core por rutas relativas (`../../core/src/...`), no por el alias, para que el alias no se resuelva en círculo.
...
- **Unit tests del core** (sin red, fixtures SDL inline con `buildSchema`): `npx vitest run src/features/graphql-orm/core/test` — **36 tests** en 5 specs que cubren el checklist de la spec:
...
- La feature GraphQL-ORM vive en `src/features/graphql-orm/` y se consume por alias (`@graphql-orm/core`, `@graphql-orm/vue`); **no** añadir deps npm de graphql-orm.
