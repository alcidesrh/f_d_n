...
describe('fieldDraft', () => {
  ...
  it('solo incluye props que aplican al tipo y descarta vacías', () => {
    const draft = { ...emptyDraft(), inputType: 'InputText' as const, name: ' nombre ', mask: '99', rows: 5 }
  ...
  it('mezcla las props avanzadas y reporta JSON inválido', () => {
    ...
    expect(propsFromDraft({ ...base, advancedJson: '{"disabled": true}' }).props).toEqual({ name: 'x', disabled: true })
