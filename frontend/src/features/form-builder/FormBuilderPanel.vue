...
import { draftFromField, emptyDraft, propsFromDraft, type DraftState } from './fieldDraft'
...
function loadFieldIntoDraft(node: BuilderField) {
  editingId.value = node.id
  propsError.value = ''
  Object.assign(draft, draftFromField(node))
}

function buildProps(): Record<string, unknown> | null {
  const result = propsFromDraft(draft)
  propsError.value = result.error ?? ''
  return result.props
}
