...
const ESTADOS: Record<EstadoJobMigracion, { label: string; severity: "success" | "info" | "warn" | "danger" }> = {
...
export const etiquetaEstado = (estado: string) => ESTADOS[estado as EstadoJobMigracion]?.label ?? estado;

export const severidadEstado = (estado: string) => ESTADOS[estado as EstadoJobMigracion]?.severity ?? "secondary";
...
export function porcentajeProgreso(job: JobMigracion): number {
  return job.total && job.total > 0 ? Math.min(100, Math.round((job.procesados / job.total) * 100)) : 0;
}
