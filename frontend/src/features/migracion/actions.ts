...
async function withNotify(action: () => Promise<unknown>, success: string, kind: "success" | "info" = "success") {
  try {
    await action();
    notify[kind](success);
  } catch (e) {
...
export const iniciarJob = (payload: PayloadEjecutar) =>
  withNotify(() => useMigracionStore().arrancarJob(payload), "Migración iniciada. Seguí el avance en la consola.");

export const reejecutarJob = (job: JobMigracion) =>
  withNotify(() => useMigracionStore().reejecutar(job), "Migración re-lanzada con los mismos parámetros.");
