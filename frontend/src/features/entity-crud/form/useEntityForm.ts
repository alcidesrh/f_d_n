...
): Promise<Record<string, AgnosticOption[]>> {
...
function columnLabels(store: Pick<EntityStore, 'columns'>): Record<string, string> {
  return Object.fromEntries(
    store.columns.filter((col) => col.label).map((col) => [col.field, col.label as string]),
  )
}
...
export function useEntityForm(
  ...
  const mode = computed<EntityFormMode>(() =>
    recordId.value != null && recordId.value !== '' ? 'update' : 'create',
  )
