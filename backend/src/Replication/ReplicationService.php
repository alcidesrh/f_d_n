...
class ReplicationService implements EventSubscriber
{
    ...
    public function syncBoletoToLegacy(int $boletoId): void
    {
        $boleto = $this->newConn->fetchAssociative(
            'SELECT b.*, s.legacy_id AS servicio_legacy_id, e.id AS estacion_id
             FROM boleto b
             JOIN salida s ON s.id = b.servicio_id
        ...
        if ($legacyBoletoId) {
            $existing = $this->fetchOldOne(
                "SELECT id FROM boleto WHERE id = :id",
                ["id" => $legacyBoletoId],
            );
            if ($existing) {
                return;
        ...
        $this->insertOld(
            ...
            [
                ...
                "cliente" => $this->getLegacyClienteId(
    ...
    private function getLegacyBusCode(?int $busId): ?string
    {
        ...
        $result = $this->newConn->fetchOne(
            "SELECT legacy_id FROM bus WHERE id = :id",
            ["id" => $busId],
        );
        return $result ?: null;
    }
