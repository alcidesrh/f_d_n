...
class MigracionCommand extends Command
{
    ...
    protected function execute(
        ...
        if ($output) {
            $output->write("<info>Salidas...</info>");
        }
        ...
        $rows = $this->fetchOld(
            "SELECT tb.*, e.id AS empresa_id FROM tarifas_boleto tb CROSS JOIN (SELECT MIN(id) AS id FROM empresa WHERE activo = 1) e AND tb.clase_asiento = 1 ORDER BY tb.id DESC",
        );
        ...
        $rows2 = $this->fetchOld(
            "SELECT tb.*, e.id AS empresa_id FROM tarifas_boleto tb CROSS JOIN (SELECT MIN(id) AS id FROM empresa WHERE activo = 1) e AND tb.clase_asiento = 2 ORDER BY tb.id DESC",
        );
